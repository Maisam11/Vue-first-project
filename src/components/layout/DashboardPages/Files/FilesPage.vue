<template>
  <div style="margin: 1rem 2.1rem 5rem 1rem">
        ...
        <div class="d-flex mb-4 align-center flex-wrap">
          <div class="d-flex align-center">
            <v-menu v-model="startDateMenu" offset-y>
              <template v-slot:activator="{ on, attrs }">
                <v-text-field
                  ...
                  dense
                  outlined
                  style="max-width: 110px; height: 35px; margin-right: 0.3rem"
                ></v-text-field>
              </template>
              <v-date-picker
                v-model="startDate"
            ...
            <v-select
              ...
              outlined
              clearable
              style="max-width: 200px; height: 35px; margin-right: 0.3rem"
        ...
        <td :colspan="headers.length">
          <v-list>
            <v-list-item
              ...
              :key="sheet.name"
              @click="viewSheet(item.id, sheet.name)"
            >
              <v-list-item-content>
                <v-list-item-title>{{ sheet.name }}</v-list-item-title>
                <v-list-item-subtitle>
                  Created: {{ formatDate(sheet.createdAt) }} | Updated:
      ...
      <v-card-text class="pa-3">
        <div class="d-flex justify-space-between align-center">
          <span class="text-caption">
            ...
            {{ pagination.endIndex }} of {{ getFilteredFiles.length }} files
          </span>
          <div class="d-flex align-center">
    ...
    <FileDialog
      ...
      v-if="canCreateFiles || canEditFiles"
    />
    <DeleteDialog
      :dialog="dialogDelete"
  ...
  computed: {
    ...
    isViewerOnly() {
      return (item) => {
        ...
        return (
          (item.viewers || []).includes(this.currentUser?.uid) ||
  ...
  methods: {
    ...
    async applyDateFilter() {
      try {
        ...
        this.showNotification(
          "Error applying date filter: " + error.message,
          "error"
        );
      }
    },
    async clearDateFilter() {
      ...
      try {
        ...
        this.showNotification(
          "Error clearing date filter: " + error.message,
          "error"
        );
      }
    },
    ...
    async applyUserFilter() {
      try {
        ...
        this.showNotification(
          "Error applying user filter: " + error.message,
          "error"
        );
      }
    },
    resetPagination() {
      ...
      this.pagination.totalPages = Math.ceil(
        this.getFilteredFiles.length / this.pagination.itemsPerPage
      );
      this.updatePaginationIndices();
    },
    ...
    openDialog(item) {
      ...
      this.editedItem = item
        ...
        : {
            ...
            addedBy:
              this.$store.getters["auth/currentUser"]?.username || "Unknown",
            sheets: [],
            editors: [this.$store.getters["auth/currentUser"]?.uid].filter(
              Boolean
            ),
            viewers: [],
          };
      ...
      for (const file of this.getFilteredFiles) {
        ...
        for (const sheet of sheets) {
          if (sheet.data && sheet.data.length > 0) {
            ...
            const sheetNameForExcel = `${file.name || `File${file.id}`}_${
              sheet.name
            }`.slice(0, 31);
